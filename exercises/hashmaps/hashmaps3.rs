...
fn build_scores_table(results: String) -> HashMap<String, Team> {
    ...
    for r in results.lines() {
        ...
        // 如果 team_1_name 不存在，则默认插入 or_insert(...)
        let mut t1 = scores.entry(team_1_name).or_insert(Team { goals_scored: 0, goals_conceded: 0 });
        t1.goals_scored += team_1_score;
        t1.goals_conceded += team_2_score;

        let mut t2 = scores.entry(team_2_name).or_insert(Team { goals_scored: 0, goals_conceded: 0 });
